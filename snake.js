...
(function () {
    ...
    function Food(food) {
        return Object.assign(food, {
            ...
            removeTimeout: null,
            spawnTimeout: null,
            drawFood: function (scene) {
                scene.drawBlock(food.position, game.blockSize, food.color);
            },
            clearFood: function (scene) {
                scene.clearBlock(food.position, game.blockSize);
            },
            ...
            scheduleSpawn: function (game) {
                ...
                food.spawnTimeout = setTimeout(function () {
                    ...
                    food.drawFood(game.scene);
                    food.scheduleRemove(game);
                }, time);
            },
            ...
            // spawnIn seconds.
            //
            scheduleRemove: function (game) {
                var time = Math.floor(Math.random() * food.removeIn * 1000);
                food.removeTimeout = setTimeout(function () {
                    food.clearFood(game.scene);
                    food.position = OUT_OF_GAME_FIELD;
                    food.scheduleSpawn(game);
                }, time);
            },

            // Clears: scheduled spawn and delete; food on the game field; food position.
            //
    ...
    game = Object.assign({
            ...
            game.foods = [
                new Food({
                    ...
                    actions: {
                        ...
                        resizeBody: function (game) {
                            return game.body.length + 3;
                        },
                        adjustSpeed: function (game) {
                            return game.speed + 1;
                        }
                    }
                }),
                new Food({
                    ...
                    removeIn: 15,
                    actions: {
                        increaseScore: function (game) {
                            return game.body.length * 2;
                        },
                        resizeBody: function (game) {
                            return game.body.length * 2;
                        },
        ...
        mainLoop: function () {
            ...
            if (food) {
                food.scheduleSpawn();
                // Actions executed after (re)spawn because action could be gameOver()
                Object.keys(food.actions).forEach(function (action) {
                    var amount = food.actions[action](game);
                    game[action](amount);
                });
                game.updateInfos();
            }
